...
class ResponseCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        if best is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best.key):
                self.false_hit_log.append({"query": query, "cached_key": best.key, "reason": "date_or_number_mismatch"})
    ...
    def similarity(a: str, b: str) -> float:
        ...
        def tokens(text: str) -> list[str]:
            ...
            grams = [word[i:i + 3] for word in words for i in range(len(word) - 2)]
            return words + grams

        va, vb = Counter(tokens(a)), Counter(tokens(b))
        dot = sum(va[token] * vb[token] for token in va.keys() & vb.keys())
        denom = math.sqrt(sum(v * v for v in va.values())) * math.sqrt(sum(v * v for v in vb.values()))
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        for key in self._redis.scan_iter(f"{self.prefix}*"):
            ...
            if score > best_score:
                best_query, best_response, best_score = cached_query, self._redis.hget(key, "response"), score
        if best_query is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best_query):
                self.false_hit_log.append({"query": query, "cached_key": best_query, "reason": "date_or_number_mismatch"})
                return None, best_score
